...
impl GastoService {
    ...
    pub async fn eliminar_gasto(&self, usuario_id: i64, id: i32) -> Result<Gasto> {
        let gasto = self.repository.find_by_id(usuario_id, id).await?
            .ok_or_else(|| anyhow!("Gasto no encontrado"))?;
    ...
    pub async fn subir_soporte(&self, usuario_id: i64, id: i32, file_data: Vec<u8>, file_name: &str) -> Result<Gasto> {
    ...
    pub async fn editar_gasto(&self, usuario_id: i64, id: i32, dto: CreateGastoDto) -> Result<Gasto> {
