...
use crate::infrastructure::database::{DatabaseManager, ProyectoRepository, PagoRepository, IngresoRepository, GastoRepository, CreditoRepository, DocumentoRepository, GastoRecurrenteRepository, UsuarioRepository};
...
pub async fn start_web_server(config: &AppConfig) -> Result<()> {
    ...
    if let Err(e) = auth_service.ensure_admin_exists().await {
        println!("⚠️ No se pudo verificar/crear usuario admin: {}", e);
    }
